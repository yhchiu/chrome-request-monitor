...
- 📌 **頁面覆蓋顯示**：目前顯示中的規則，找到匹配的URL時會在頁面上顯示通知框
- 👁️ **規則顯示範圍**：可選擇哪些規則要顯示在彈出窗口的列表與頁面通知框中；未選取的規則仍然會被記錄下來
...
設定完規則後，當您瀏覽網站時：
- 符合規則的URL都會被記錄下來，其中目前顯示中的規則會在頁面右上角顯示通知框（預設為全部規則）
- 點擊通知框中的「複製」按鈕可複製URL
- 點擊「關閉」按鈕可關閉通知框，或點擊「全部關閉」一次關掉頁面上所有通知框

### 第三步：查看歷史記錄

- 點擊擴展圖標打開彈出窗口
- 使用「顯示規則：」列表挑選想看的規則，它同時決定下方列表與頁面上懸浮視窗的顯示範圍；未勾選的規則仍然會被記錄，重新勾選後就會出現
- 一個 URL 可能同時符合多條規則。只要其中任何一條有勾選就會顯示，並且會列出所有符合的規則
- 預設只顯示目前分頁的URL，取消勾選「只顯示目前分頁」可查看所有分頁的URL
- 清單會自動跟上新抓到的URL。閱讀時若不想被打斷，可取消勾選「自動更新」；期間抓到的URL仍然會被記錄，重新勾選或按「刷新」就會補上
- 查看找到的URL列表
- 點擊「複製URL」按鈕複製特定URL
- 使用「清除全部」按鈕清空歷史記錄
...
- 資料最小化與控制：所有處理皆在本機完成。規則/設定儲存於 Chrome 同步儲存；目前顯示中的規則只留在本機，不會同步；找到的 URL 僅在工作階段暫存，且可隨時清除。您可切換監控，並選擇僅顯示目前分頁結果。

## 🔒 隱私說明

- 此擴展僅在本地處理數據，不會上傳任何信息到外部伺服器
- 用戶設定的規則存儲在Chrome同步存儲中
- 目前顯示中的規則只存放在本機的 local 儲存空間，不會跟著您到另一台電腦
